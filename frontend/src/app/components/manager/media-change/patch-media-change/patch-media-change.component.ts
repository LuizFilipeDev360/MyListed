...
@Component({
  selector: 'app-patch-media-change',
  standalone: true,
  imports: [FormsModule],
  templateUrl: './patch-media-change.component.html',
  styleUrl: './patch-media-change.component.css'
})
export class PatchMediaChangeComponent {
        ...
        patchMedia(){
          ...
          this.media.title = this.media.title?.trim() || null;
          this.media.year = this.media.year !== null ? Number(this.media.year) : null;
          this.media.kind = this.media.kind !== null ? Number(this.media.kind) : null;
          this.media.description = this.media.description?.trim() || null;
          this.media.imageUrl = this.media.imageUrl?.trim() || null;

          if (!this.media.genreIds || this.media.genreIds.length === 0) {
            this.media.genreIds = null;
          }

          this.mediaApi.patchMedia(this.id,this.media).subscribe({
            next: (res) => {
              console.log(res);
            },
            error: (err) => {
